    ...
    if request.method == 'POST':
        if 'register_user' in request.POST:
            ...
            if data.filter(phone=phone).exists():
                ...
                x = ("CREATE TABLE user{} (id INT AUTO_INCREMENT PRIMARY KEY,product_img VARCHAR(100), product_Name VARCHAR(100), product_price VARCHAR(100), product_details VARCHAR(100))").format(phone)
                mycursor.execute(x)
                connection.commit()
